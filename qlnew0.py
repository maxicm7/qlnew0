...
if df is not None:
    ...
    if historical_combinations_set:
        col_freq, col_spec, col_delay_cv = st.columns(3)
        with col_freq:
...
if df is not None:
    ...
    with run_col2:
        if st.button("Ejecutar Simulación en Cascada"):
            ...
            # --- LÓGICA CORREGIDA ---
            combinaciones_refinadas = todas_unicas # Por defecto, son todas las únicas
            ...
            if combinaciones_refinadas:
                ...
                for c in combinaciones_refinadas:
                    ...
                    data.append({
                        ...
                        "CV Atraso": np.std(delays)/np.mean(delays) if np.mean(delays) > 0 else 0,
...
st.sidebar.header("Guía del Modelo"); st.sidebar.markdown("Este modelo se basa en el **principio de homeostasis**: un sistema aleatorio tiende al equilibrio."); st.sidebar.markdown("**Filtros de Homeostasis y Estructura (Etapa 1):**"); st.sidebar.markdown("- **CV Frecuencia:** Equilibrio a largo plazo. - **Cálculo Especial:** Equilibrio a corto plazo. - **CV Atraso:** Equilibrio interno de la combinación."); st.sidebar.markdown("**Filtro Estratégico (Etapa 2):**"); st.sidebar.markdown("- **Composición:** Define la 'personalidad' (Calientes, Fríos, etc.). La app **recomienda** la estrategia más común del historial.")
